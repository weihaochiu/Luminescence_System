...
from .window import CameraLinearityQualificationWindow


def main() -> int:
    QCoreApplication.setOrganizationName("EL Measurement Lab")
    QCoreApplication.setApplicationName("Camera Linearity Qualification")
    app = QApplication(sys.argv)
    app.setStyle("Fusion")
    _configure_logging()
    configure_i18n(QSettings("EL Measurement Lab", "EL Measurement Equipment Control"))
    window = CameraLinearityQualificationWindow()
    window.show()
    return app.exec()
