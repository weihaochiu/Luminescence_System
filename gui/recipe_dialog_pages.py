...
class RecipeDialogPagesMixin:
    ...
    def _build_polarity_tab(self) -> QWidget:
        ...
        self.light_control_combo = QComboBox()
        self.light_control_combo.addItem("人工開／關白光提示", "manual")
        self.light_control_combo.addItem("自動光源控制（預留）", "automatic")
        self.light_stable_spin = _double_spin(0, 600, 2, " s")
        self.dark_recovery_spin = _double_spin(0, 3600, 5, " s")
        self.polarity_nplc_spin = _double_spin(0.001, 100, 1, " NPLC")
        self.polarity_samples_spin = QSpinBox()
        self.polarity_samples_spin.setRange(1, 1000)
        self.voc_min_spin = _double_spin(-10, 10, 0.2, " V")
        self.voc_max_spin = _double_spin(-10, 10, 1.5, " V")
        self.jsc_min_spin = _double_spin(-1000, 1000, 1, " mA/cm²")
        self.jsc_max_spin = _double_spin(-1000, 1000, 40, " mA/cm²")
        self.polarity_failure_combo = QComboBox()
        self.polarity_failure_combo.addItem("要求人工確認", "confirm")
        self.polarity_failure_combo.addItem("中止量測", "abort")
        form.addRow(self.polarity_required_check)
        form.addRow("白光控制", self.light_control_combo)
        form.addRow("白光穩定等待", self.light_stable_spin)
        form.addRow("關燈後暗態恢復", self.dark_recovery_spin)
        form.addRow("NPLC", self.polarity_nplc_spin)
        form.addRow("Voc／Jsc 各取樣數", self.polarity_samples_spin)
        form.addRow("Voc 合理下限", self.voc_min_spin)
        form.addRow("Voc 合理上限", self.voc_max_spin)
        form.addRow("|Jsc| 合理下限", self.jsc_min_spin)
        form.addRow("|Jsc| 合理上限", self.jsc_max_spin)
        form.addRow("無法判定時", self.polarity_failure_combo)
        note = QLabel("目前採人工白光提示；程式只保存流程與判定條件，不會自動控制燈源。")
